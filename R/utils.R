iupac_to_glycoct <- function(
  x,
  java_app_path = "/Users/fubin/GlycanFormatConverter-cli"
) {
  ...
  if (!dir.exists(java_app_path)) {
    cli::cli_abort(
      "Java application directory not found: {.path {java_app_path}}"
    )
  }
...
convert_iupac_to_wurcs <- function(iupac_str, jar_path) {
  ...
  if (length(result) == 0) {
    cli::cli_abort(
      "Failed to convert IUPAC to WURCS: No output from Java application"
    )
  }
...
convert_wurcs_to_glycoct <- function(wurcs_str, jar_path) {
  ...
  if (length(result) == 0) {
    cli::cli_abort(
      "Failed to convert WURCS to GlycoCT: No output from Java application"
    )
  }
  ...
  if (length(start_idx) == 0) {
    cli::cli_abort(
      "Failed to convert WURCS to GlycoCT: No GlycoCT output found"
    )
  }
